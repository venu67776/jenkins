def nexus(COMPONENT) {
  ...
  command = "curl -f -v -u admin:admin123 --upload-file ${FILENAME} http://172.31.3.145:8081/repository/${COMPONENT}/${FILENAME}"
...
def make_artifacts(APP_TYPE, COMPONENT) {
  ...
  if(APP_TYPE == "NGINX") {
    ...
    command = "zip -r ${FILENAME} main.go tracing.go user.go "
    def execute_com=sh(returnStdout: true, script: command)
    print execute_com
  }
}
