def nexus(COMPONENT) {
  ...
  command = "curl -f -v -u admin:admin --upload-file ${FILENAME} http://172.31.14.124:8081/repository/${COMPONENT}/${FILENAME}"
...
def make_artifacts(APP_TYPE, COMPONENT) {
  ...
  if(APP_TYPE == "NGINX") {
    ...
    command = "zip -r ${FILENAME} main.go user.go tracking.go"
    def execute_com=sh(returnStdout: true, script: command)
    print execute_com
  }
}
